	...
	public:
		...
		void bgp_update(bool u_sign=0){
			...
			if(!OldPathVector.Validity)
				...
				MarkedID = OldPathVector.LastHop;
			}
			...
			if(ChangedInThisLoop){
				...
				NewPathVector.Path.push_back(ID);
				NewPathVector.LastHop = MarkedID;
				IsChanged = 1;
			}
		}
};
...
void myprint_path(){
	for( u_s i = 0; i < NodeNum; i ++){
		...
		if(! AllNodeVector[i]->NewPathVector.Validity)
			...
			for(vector<u_s>::iterator itor = AllNodeVector[i]->NewPathVector.Path.begin(); itor != AllNodeVector[i]->NewPathVector.Path.end(); itor++)
...
void create_adj( ){
	...
	for( u_s i = 0; fgets(IBuffer, 70000, fp); i ++){
		tok = strtok(IBuffer, " ");
		SortVector[i] = atoi(tok);
	}
	fclose(fp);
//	for(u_s i = 0; i < NodeNum; i ++)
