	...
	//	bool Extensible;
		...
		u_s set(u_s level){
			...
			for(u_s i = 0; i < level; i ++){
				...
				for( u_s j = 0, nei = 0; j < AllNodeVector[CurrentID]->AdjList.size(); j ++){
					nei = AllNodeVector[CurrentID]->AdjList[j];
					if( AllNodeVector[nei]->IsInSDN){
						AllNodeVector[CurrentID]->IsInSDN = AllNodeVector[nei]->IsInSDN;
						break;
					}
				}
...
//	for( u_s i = 0; i < 100; i ++)
//	{
//		fprintf(stderr," SortVector[%d]: %d degree: %d + ",i, SortVector[i], AllNodeVector[SortVector[i]]->AdjList.size()); 
//	}
	//	return int(AllNodeVector[*(u_s *)b]->AdjList.size()) - AllNodeVector[*(u_s *)a]->AdjList.size();
	//SortVector[0] = SortVector[Serial];
//	fprintf(stderr," \nswap: Serial: %d - %d\n", Serial, SortVector[0]); 
